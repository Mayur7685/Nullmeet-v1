...
import { TEE_RPC_URL, TEE_VALIDATOR } from "@/lib/constants";
...
export function MeetingResult({
  ...
  const generateCalendarLink = (): string => {
    ...
    const details = meetingAccount
      ? `Meeting+scheduled+via+Nullmeet+(Solana+TEE)%0AOn-chain+proof:+https://explorer.solana.com/address/${meetingAccount}?cluster=devnet`
      : `Meeting+scheduled+via+Nullmeet+(Solana+TEE)`;

    return `https://calendar.google.com/calendar/render?action=TEMPLATE&text=Private+Meeting&dates=${dateStr}T${startHour}0000/${dateStr}T${endHour}0000&details=${details}`;
  };

  if (!validOverlap || resultSlot === null) {
    return (
      <div className="text-center space-y-4">
        <div className="text-4xl">:(</div>
        <div className="text-2xl font-bold text-[var(--error)]">No Overlap Found</div>
        <p className="text-[var(--muted)]">
          Neither party has any overlapping available time slots. Try again with
  ...
  return (
    <div className="text-center space-y-6">
      ...
      <div className="p-6 bg-[var(--card)] border border-[var(--success)]/30 rounded-xl">
        <div className="text-sm text-[var(--muted)] mb-2">Best time slot</div>
        <div className="text-3xl font-bold text-[var(--foreground)]">
          {TIME_LABELS[resultSlot]}
        </div>
        <div className="text-sm text-[var(--muted)] mt-2">
          Compatibility score: {resultScore}
        </div>
      </div>
      ...
      <a
        ...
        className="inline-flex items-center gap-2 px-5 py-3 bg-purple-600 hover:bg-purple-500 active:scale-95 text-white font-semibold rounded-lg transition-all cursor-pointer"
      >
      ...
      <p className="text-sm text-[var(--muted)] max-w-sm mx-auto">
        This result was computed inside a TEE enclave. Neither party&apos;s full
      ...
      <div className="flex flex-col items-center gap-3">
        {meetingAccount && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-4 py-2 text-sm text-purple-500 hover:text-purple-400 active:scale-95 border border-purple-500/30 hover:border-purple-500/60 rounded-lg transition-all cursor-pointer"
          >
        ...
        <div className="pt-2 border-t border-[var(--border)]">
          {teeVerified === null ? (
            <button
              ...
              disabled={verifying}
              className="w-full py-2 text-sm text-purple-500 hover:text-purple-400 active:scale-95 border border-purple-500/30 hover:border-purple-500/60 rounded-lg transition-all cursor-pointer disabled:opacity-50 disabled:active:scale-100"
            >
